...
async function handleFormSubmit() {
  ...
  // 3. Mengubah Tombol Menjadi Loading Spinner
  btn.disabled = true;
  btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i><span>Mengirim...</span>';

  // 4. Menyusun Data Formulir (GANTI ACCESS KEY DI BAWAH INI)
  const formData = {
    access_key: "7f057625-1d1d-4085-a3f5-aac69c732606", // <-- Tempelkan kode dari email kamu di sini
    name: name.value.trim(),
    email: email.value.trim(),
    subject: subject?.value.trim() || "Pesan Baru dari Portofolio Web",
    message: message.value.trim()
  };

  try {
    // 5. Mengirimkan Data ke Server Web3Forms
    ...
    // 6. Jika Server Merespons Sukses
    if (result.success) {
      ...
      if (form && success) {
        ...
        success.style.display = 'flex'; // Tampilkan pesan sukses
      }
      
      // Opsional: Mengosongkan kembali isi input form
      if(form) form.reset();
      
    } else {
      // Jika server mendeteksi error (misal Access Key salah)
      alert("Gagal mengirim pesan: " + result.message);
      shakeButton(btn);
    }
  } catch (error) {
    // Jika koneksi internet terputus atau server down
    alert("Terjadi kesalahan jaringan. Silakan coba lagi nanti.");
    shakeButton(btn);
  } finally {
    // 7. Mengembalikan Tombol ke Tampilan Semula
    btn.disabled = false;
